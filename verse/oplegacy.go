...
func (op *transactableOPLegacy) Transact(
	...
	vc, err := sccverifier.NewOasysRollupVerifier(op.VerifyContract(), op.L1Signer())
	if err != nil {
		return nil, err
	}
...
func findStateBatchAppendedEvent(
	...
	iter, err := sc.FilterStateBatchAppended(opts, []*big.Int{new(big.Int).SetUint64(batchIndex)})
	if err != nil {
		return nil, err
	}
	defer iter.Close()
