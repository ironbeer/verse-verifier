...
func (s *OptimismSignatureDBTestSuite) TestFind() {
	...
	for _, c := range cases {
		s.Run(c.name, func() {
			gots, _ := s.db.Find(c.idAfter, c.signer, c.contract, c.index, c.limit, c.offset)
			assert(gots, c.wants)
		})
	}
}
...
func (s *OptimismSignatureDBTestSuite) TestSave() {
	...
	// batchSize := uint64(10)
	// prevTotalElements := uint64(20)
	// extraData := []byte("test")
	approved := true
	signature := RandSignature()

	got0, _ := s.db.Save(nil, nil,
		signer.Address, contract.Address, batchIndex, batchRoot, approved, signature)
	ulid.MustParse(got0.ID)
	s.Equal("", got0.PreviousID)
	s.Equal(*signer, got0.Signer)
	s.Equal(*contract, got0.Contract)
	s.Equal(uint64(1), got0.RollupIndex)
	s.Equal(batchRoot, got0.RollupHash)
	// s.Equal(batchSize, *got0.BatchSize)
	// s.Equal(uint64(prevTotalElements), *got0.PrevTotalElements)
	// s.Equal(extraData, *got0.ExtraData)
	s.Equal(approved, got0.Approved)
	s.Equal(signature, got0.Signature)
	...
	got0, _ = s.db.Save(nil, nil,
		signer.Address, contract.Address, batchIndex, rollupHash, approved, signature)
	ulid.MustParse(got0.ID)
	s.Equal("", got0.PreviousID)
	s.Equal(*signer, got0.Signer)
	s.Equal(*contract, got0.Contract)
	s.Equal(uint64(1), got0.RollupIndex)
	s.Equal(rollupHash, got0.RollupHash)
	// s.Nil(got0.BatchSize)
	// s.Nil(got0.PrevTotalElements)
	// s.Nil(got0.ExtraData)
	s.Equal(approved, got0.Approved)
	s.Equal(signature, got0.Signature)

	// add new
	got1, _ := s.db.Save(nil, nil,
		signer.Address, contract.Address, batchIndex+1, batchRoot,
		approved, signature)
	s.Equal(got0.ID, got1.PreviousID)

	// other signer
	got2, _ := s.db.Save(nil, nil,
		s.RandAddress(), contract.Address, batchIndex+2, batchRoot,
		approved, signature)
	s.Equal("", got2.PreviousID)

	// overtaking test
	id1, id2 := util.ULID(nil).String(), util.ULID(nil).String()
	_, err := s.db.Save(&id1, &id2,
		s.RandAddress(), contract.Address, batchIndex+3, batchRoot,
		approved, signature)
